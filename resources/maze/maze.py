...
class Maze(object):
    ...
    def __init__(self, mazePath):
        ...
        self.pelletRemaining = 0
        self.levelObjects = [[levelObject("empty") for j in range(27)] for i in range(48)]
        ...
        self.levelObjectNamesPassable = ["empty", "pellet", "power", "fruit"] #add in the future
        ...
        for levelLine in levelFile.readlines():
            ...
            for i in range(48):
                ...
                if levelLineSplit[i] == "_":    # passage
                    ...
                    self.levelObjects[i][levelLineNo].name = "fruit"
...
class movingObject(object):
        ...
        if self.isCaged == True:    # if ghost is caged, prevent the movement
            ...
            try:
                for i in range(4):
                    ...
                    if i == 0:
                        nextObject = Maze.levelObjects[self.coordinateRel[0]-1][self.coordinateRel[1]] # levelObject on the left
                    elif i == 1:
                        nextObject = Maze.levelObjects[self.coordinateRel[0]+1][self.coordinateRel[1]] # levelObject on the right
                    elif i == 2:
                        nextObject = Maze.levelObjects[self.coordinateRel[0]][self.coordinateRel[1]-1] # levelObject on the up
                    elif i == 3:
                        nextObject = Maze.levelObjects[self.coordinateRel[0]][self.coordinateRel[1]+1] # levelObject on the down          
        ...
        if self.dirNext == self.dirCurrent: # in this case, no action is required
            ...
            if self.dirNext == "Left":  # check the direction first
                ...
                if self.coordinateRel[0] == 0: # at left edge, allow to change direction without checking (prevent index error)
                    ...
                    nextObject = Maze.levelObjects[self.coordinateRel[0]-1][self.coordinateRel[1]] # levelObject placed left of this object
                ...
                if self.coordinateRel[0] == 47: # at right edge, allow to change direction without checking (prevent index error)
                    self.dirCurrent = "Right"

                else:
                    nextObject = Maze.levelObjects[self.coordinateRel[0]+1][self.coordinateRel[1]] # levelObject placed right of this object
                ...
                if self.coordinateRel[1] == 26: # at bottom edge, allow to change direction without checking (prevent index error)
                    self.dirCurrent = "Down"

                else:
                    nextObject = Maze.levelObjects[self.coordinateRel[0]][self.coordinateRel[1]+1] # levelObject placed down of this object
                ...
                if self.coordinateRel[1] == 0: # at top edge, allow to change direction without checking (prevent index error)
                    ...
                    nextObject = Maze.levelObjects[self.coordinateRel[0]][self.coordinateRel[1]-1] # levelObject placed up of this object
    ...
    def MoveCurrent(self, Maze):
        ...
        if self.dirCurrent == "Left":
            ...
            if self.coordinateAbs[1] == 26*4:  # at bottom edge, move to top edge
            ...
            if self.coordinateAbs[1] == 0:  # at top edge, move to bottom edge
                self.coordinateAbs[1] = 26*4 + 3
                self.coordinateRel[1] = 27
                self.dirEdgePassed = True
